...
def scenario_outfit(ax, color=RGB_to_Hex('202,202,202')):
    ...
    # --- NEW: Pedestrian Crosswalk Visuals (Tailored for Intersection) ---
    cw_style = ':'
    cw_color = 'gray'
    cw_width = 1.5
    
    # South crosswalk
    ax.plot([-10, 10], [-12.5, -12.5], c=cw_color, linestyle=cw_style, linewidth=cw_width)
    ax.plot([-10, 10], [-15, -15], c=cw_color, linestyle=cw_style, linewidth=cw_width)
    # North crosswalk
    ax.plot([-10, 10], [12.5, 12.5], c=cw_color, linestyle=cw_style, linewidth=cw_width)
    ax.plot([-10, 10], [15, 15], c=cw_color, linestyle=cw_style, linewidth=cw_width)
    # East crosswalk
    ax.plot([12.5, 12.5], [-10, 10], c=cw_color, linestyle=cw_style, linewidth=cw_width)
    ax.plot([15, 15], [-10, 10], c=cw_color, linestyle=cw_style, linewidth=cw_width)
    # West crosswalk
    ax.plot([-12.5, -12.5], [-10, 10], c=cw_color, linestyle=cw_style, linewidth=cw_width)
    ax.plot([-15, -15], [-10, 10], c=cw_color, linestyle=cw_style, linewidth=cw_width)
...
# --- NEW: Pedestrian Splines and Caching (Tailored for Intersection) ---
...
def pedestrian_ref_line(crosswalk_loc, direction):
    ...
    """
    if crosswalk_loc == 's':
        x = np.linspace(-10, 10, 1000) if direction == 1 else np.linspace(10, -10, 1000)
        y = -13.75 * np.ones_like(x)
    elif crosswalk_loc == 'n':
        x = np.linspace(-10, 10, 1000) if direction == 1 else np.linspace(10, -10, 1000)
        y = 13.75 * np.ones_like(x)
    elif crosswalk_loc == 'e':
        y = np.linspace(10, -10, 1000) if direction == 1 else np.linspace(-10, 10, 1000)
        x = 13.75 * np.ones_like(y)
...
def default_pedestrian_state(crosswalk_loc, direction):
    ...
    max_speed = 3.0
    if crosswalk_loc == 's':
        state = [-10.0, -13.75] if direction == 1 else [10.0, -13.75]
        heading = 0.0 if direction == 1 else math.pi
    elif crosswalk_loc == 'n':
        state = [-10.0, 13.75] if direction == 1 else [10.0, 13.75]
        heading = 0.0 if direction == 1 else math.pi
    elif crosswalk_loc == 'e':
        state = [13.75, 10.0] if direction == 1 else [13.75, -10.0]
        heading = 1.5 * math.pi if direction == 1 else 0.5 * math.pi
    elif crosswalk_loc == 'w':
        state = [-13.75, -10.0] if direction == 1 else [-13.75, 10.0]
        heading = 0.5 * math.pi if direction == 1 else 1.5 * math.pi
    
    dis2des = 20.0 # Total crosswalk distance (20m across the intersection)
...
def record_all_pedestrian_ref_line():
    ...
    for loc in possible_locs:
        ...
        for d in directions:
            ...
            # We use Euclidean distance for pedestrians, so we just compute arc length here
            gap_list = np.zeros(len(ref_line))
            for point in range(len(ref_line) - 1):
                gap = np.sqrt((ref_line[point, 0] - ref_line[point + 1, 0]) ** 2 +
                              (ref_line[point, 1] - ref_line[point + 1, 1]) ** 2)
                gap_list[point:] += gap
            ped_gap_list[loc][d] = np.flipud(gap_list)
