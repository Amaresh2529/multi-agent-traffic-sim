...
class Bayesian_Agent:
    """
    Solves 1v1 nash game. 
    In Multi-Agent mode, this is instantiated sequentially for each tracking target 
    to determine their independent actions relative to the Ego vehicle.
    """
    def __init__(self, hdv_info, cav_info, action_type):
        ...
        # --- NEW: Expanded 5-State Bayesian Probability Distribution ---
        self.aggressiveness_distribution = [0.10, 0.35, 0.35, 0.10, 0.10]  # ['agg', 'nor', 'con', 'hes', 'dis']
    ...
    def get_max_speed(self):
        if self.ego_aggressiveness == 'agg':
            ...
            return Target_speed[2] * 0.7  # Hesitant drivers have a much lower top speed cap
        elif self.ego_aggressiveness == 'dis':
            return Target_speed[1]  # Distracted drivers drift around the normal speed cap
        else:
    ...
    def reward_weight(self, aggressiveness):
        if aggressiveness == 'agg':
            ...
            reward_weight = Weight_hv[2]  # Weighted toward conservative preservation
        elif aggressiveness == 'dis':
            reward_weight = Weight_hv[1]  # Erratically weighted near normal behavior
        else:
            reward_weight = Weight_hv[1]
        return reward_weight

    def get_ttc_thr(self, aggressiveness):
        # --- NEW: Expanded Danger Thresholds ---
        if aggressiveness == 'agg':
            ttc_thr = 2
        elif aggressiveness == 'nor':
            ttc_thr = 6
            ...
            ttc_thr = 9  # Spooked easily, reacts to targets far away
        elif aggressiveness == 'dis':
            ttc_thr = 3  # Not paying attention, reacts dangerously late
        else:
            ttc_thr = 6
        return ttc_thr
    ...
    def update_state(self, output_action=False): 
        if not tools.if_passed_conflict_point(self.ego_info, self.inter_info):
            ...
            # --- NEW: Iterate through all 5 psychological profiles to calculate Nash matrix ---
            for inter, inter_aggressiveness in enumerate(['agg', 'nor', 'con', 'hes', 'dis']):
                ...
                for ego_action in range(Action_length):
                    r[ego_action] += self.aggressiveness_distribution[inter] * \
                                     self.reward(ego_action, inter_pure_strategy, inter_vehicle_aggressiveness=inter_aggressiveness)[0] 
